...
///var ///location = "-23.596898082025685, -46.66440725326538"
		...
		function panorama_init( lat,lng ) {
			...
			pano_canvas = document.createElement( 'canvas' );
			...
			pano_client.getPanoramaByLocation( lat_lng, 500, function( result, status ) {
				if (status == google.maps.StreetViewStatus.OK) {
					load_pano_image(result.location.pano)
					//build_panorama( result.location.pano, 2 );
					//console.log( result );

				} else {					
					alert("Could not retrieve panorama for the following reason(s): " + status);
				}
			} );
...
function load_pano_image(id) {
	
	url = 'http://client.heliozilla.com/highrise_million/platform_working_copy_clone/php/curl_proxy_stitch.php?id='+id 
...
function updateProgress(evt) {
  ...
  if (evt.lengthComputable) {
	...
	//set_debugger("Loading Intro Video: "+ Math.round(percentComplete) + "% of " + Math.round(evt.total/1000) + " kBytes");
...
function transferCanceled(evt) {
  alert("The transfer has been canceled by the user.");
	...
	function pano_mesh_build(url) {
			...
			street_mesh = new THREE.Mesh( pano_geometry, sv_mat);
		...
		function pano_texture_apply() {
			...
			pano_mesh_texture.needsUpdate = true
		    var pano_geometry = new THREE.Sphere( 7700, 20, 20 )
			street_mesh = new THREE.Mesh( pano_geometry, new THREE.MeshBasicMaterial({ map:pano_mesh_texture} ));
			street_mesh.flipSided = true;
			scene.addObject( street_mesh );	
			street_mesh.visible=true
...
function build_panorama( id, zoom ) {
			...
			for( var y = 0; y < h; y++ ) {
				for( var x = 0; x < w; x++ ) {
					var tile_url = encodeURIComponent('http://maps.google.com/cbk?output=tile&panoid=' + id + '&zoom=' + zoom + '&x=' + x + '&y=' + y);
					var _url = 'http://client.heliozilla.com/highrise_million/platform_working_copy/php/curl_proxy.php?url='+ tile_url;
					var img = new Image();
